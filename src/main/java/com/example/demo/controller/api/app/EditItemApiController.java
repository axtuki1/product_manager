...
public class EditItemApiController {
	...
	public HashMap<String, Object> viewPage(Model model, HttpSession session,
		...
		if (form.getName().equals("") || form.getPrice() < 0 || form.getGenre() < 0 || form.getAmount() < 0) {
			return null;
		} else {
			repository.save(Item.genItem(id, form.getName(), form.getPrice(), form.getAmount(), form.getGenre(),
